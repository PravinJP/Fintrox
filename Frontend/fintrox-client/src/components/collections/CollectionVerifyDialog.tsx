...
const CollectionVerifyDialog: React.FC<CollectionVerifyDialogProps> = ({
  isOpen,
  onClose,
  onConfirm,
  collection,
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-sm overflow-hidden text-center p-6 space-y-4">
        ...
        <div>
          <h3 className="font-bold text-slate-900 text-lg">Verify Collection</h3>
          <p className="text-xs text-slate-500 mt-1">
            Verify collection <span className="font-semibold text-slate-700">{collection.collectionNumber}</span>{' '}
            of ₹{collection.amount?.toLocaleString()}?
          </p>
        </div>
...
export default CollectionVerifyDialog;